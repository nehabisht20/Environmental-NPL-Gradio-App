    ...
    def load_image_gen_model(self):
        if self.image_gen_model is None:
            try:
                ...
                # Fallback to a smaller model if GPU memory is limited or other issues
...
def generate_image(text, category):
    ...
    try:
        if hasattr(model_manager.image_gen_model, 'to'):
            ...
            # Using pipeline
            image = model_manager.image_gen_model(enhanced_prompt)[0]['generated_image']
        return image
    except Exception as e:
    ...
    try:
        ...
        if isinstance(predictions, list):
            for i, pred in enumerate(predictions[:3]):  # Show top 3
             ...
             filled_text = predictions['sequence']
             confidence = predictions['score']
             result_text += f"1. {filled_text} (Confidence: {confidence:.3f})\n"
